...
\  DUP 1 H.R>S SSPACE \ печать адреса перехода
               NEAR_NFA
               >R DUP     
   IF  \ .S" @"
   \ IF сюда проверку ELSE

 R@ DUP   START-WORD @ <  >R END-WORD @ > R> OR R@ START-WORD @ =  OR  0=  IF  R>   DUP  ADD-LAB  .S" @" 1 H.R>S  ELSE  
\  .S" @" 
  ADR@ @ R@ =  IF 0 ADR@ ! ELSE R@ ADD-LAB  THEN  \ .S" 2@" <--
   DUP   NAME !     \    COUNT >S \ печать скобок. «нак метки
                 NAME> DUP ADD-CALL2   \ без ADD-CALL не работают ссылки на переменные ADD-LAB CR WordByAddr TYPE CR
           (   DUP  H.>Sn  )     R@ -  
...
RDROP   NAME @  COUNT >S .S" ]"
 ELSE 
.S" @@"  RDROP NAME @  COUNT >S THEN   \ конец проверки
               THEN    
                   DROP      \  .S"  ) "
                ELSE   R> .S" ["   H.>S .S" ]"  DROP  \ тут 
                THEN
                ;
...
\ IMAGE-BEGIN - попробовать вмеместо метки OVER @  IMAGE-BEGIN > IF ." DD" OVER @ . HERE . THEN
...
: INST  ( ADR -- ADR' )  
        ...
        ELSE    DUP DIS-OP
                OVER BASE-ADDR -  6
             \  H.R SPACE    \ печать адреса метки по условию, 2DROP убрать
             \    2DROP \  H.R SPACE адреса
                    OVER TYPE-LAB 
           IF  2DROP ELSE 
      OVER LAB   IF 0 TRIGER3 ! 2DROP ELSE S" @" TYPE H.R S" :" TYPE SSPACE  THEN
           THEN
               ...
               2DROP \  DO I C@ 2 SPACES DROP ( H.N ) LOOP  \  2DROP ( H.N ) LOOP \  дамп команды
                R> 12 <    IF 9 EMIT THEN
		\ NEXT-INST C@ 0xE8 =
               \ IF  NEXT-INST 1+ @+ SWAP + 
                 \   CASE
           ...
           \        ['] _CONSTANT-CODE OF  0  ENDOF
             \      ['] _USER-CODE     OF 0  ENDOF
              \     ['] _CREATE-CODE   OF  0 ." // ќформит как var или label: array[0..10] of Byte/Integer;" ENDOF
               \    ['] _USER-VALUE-CODE OF  0 ." // ќформит как" ENDOF
